...
vector<vector<point> > f2(double a, double b, double c, vector<vector<point> > all) {
  vector<vector<point> > newAll;
  vector<point> newP;
  // cout << a << " " << b << " " << c << endl;

  // if (abs(b - 0.0) <= EPS) {
  //   // newP = cutPolygon({-1.0 * c / a, 0.0}, {-1.0 * c / a, 10.0}, P);
  //   cout << "b is 0\n";
  //   cout << (-1.0 * c / a) << " " << (0.0) << " " << (-1.0 * c / a) << " " << (10.0) << endl;
  // } else {
  //   cout << 0 << " " << (-1.0 * c / b) << " " << 1 << " " << (-1 * (c + 10.0 * a) / b) << endl;
  // }
  ...
  for (auto &P: all) {
    ...
    if (abs(b - 0.0) <= EPS) {
      ...
      newP = cutPolygon({0.0, -1.0 * c / b}, {10.0, -1.0 * (c + 10.0 * a) / b}, P);
    }
    
    if (newP.size() > 0) {
      newAll.push_back(newP);
    }
    ...
    if (abs(b - 0.0) <= EPS) {
      newP = cutPolygon({-1.0 * c / a, 10.0}, {-1.0 * c / a, 0.0}, P);
    } else {
      newP = cutPolygon({10.0, -1.0 * (c + 10.0 * a) / b}, {0.0, -1.0 * c / b}, P);
    }
    if (newP.size() > 0) {
      newAll.push_back(newP);
    }
  ...
  // cout << "size: " << newAll.size() << endl << endl;
  return newAll;
}

string f3( vector<point> candles, vector<vector<double > > lines, vector<vector<point> > all) {
  for (auto &line: lines) {
    double a = line[0], b = line[1], c = line[2];
    all = f2(a, b, c, all);
  }

  if (all.size() != candles.size()) {
    return "no";
  }

  // cout << "FINAL\n";
  vector<int> count(all.size(), 0);
  for (int i = 0; i < all.size(); ++i) {
    for (auto &candle: candles) {
      if (insidePolygon(candle, all[i]) == 1) {
        // cout <<"CANDLE IS INSIDE POLYGON\n";
        // cout << candle.x << " " << candle.y << " " << i << endl;
        // for (auto &p: all[i]) {
        //   cout << p.x << " " << p.y << endl;
        // }
        // cout << endl;
...
vector<point> generateCircle(double R) {
  double n = 100000;
  ...
  for (double i = 0; i < n; ++i) {
    theta += dtheta;
    res.emplace_back(R*sin(theta), R*cos(theta));
  }
  res.push_back(res[0]);
  return res;
}
...
int main() {
  ...
  int N, M; double R, a, b, c;
  cin >> N >> M >> R;
  // cout << R << endl;
  initialPolygon = generateCircle(R);
  // initialPolygon.emplace_back(R, R);
  // initialPolygon.emplace_back(R, -1.0 * R);
  // initialPolygon.emplace_back(-1.0 * R, -1.0 * R);
  // initialPolygon.emplace_back(-1.0 * R, R);
  // initialPolygon.emplace_back(initialPolygon[0]);

  // for (auto &p: initialPolygon) {
  //   cout << p.x << " " << p.y << endl;
  // }
  // cout << endl;
  ...
  all.push_back(initialPolygon);

  // P = cutPolygon({0, 2}, {1, 1}, P);
  // for (auto &p: P) {
  //   cout << p.x << " " << p.y << endl;
  // }  

  cout << f3(candles, lines, all);
  // f2(lines, candles);
}